...
export default function PayoutDashboard({account_id, refreshCount}:{account_id: string, refreshCount: number}) {
    const { updateModal } = useModalStore();

    const [isLoading, setIsLoading] = useState(false);
    const [balance, setBalance] = useState();
    const [transactions, setTransactions] = useState<TransactionModelSchemaTypes & { createdAt: any; updatedAt: any }[] | []>([]);
    ...
    if(isLoading)return(
        <View style={{gap: 20, opacity: 0.7}}>
            <View style={{height: 200, backgroundColor: colors.grey50, borderRadius: 15}} />
            <View style={{gap: 10}}>
                <View style={{height: 50, backgroundColor: colors.grey50, borderRadius: 10}} />
                <View style={{height: 50, backgroundColor: colors.grey50, borderRadius: 10}} />
                <View style={{height: 50, backgroundColor: colors.grey50, borderRadius: 10}} />
            </View>
        </View>
