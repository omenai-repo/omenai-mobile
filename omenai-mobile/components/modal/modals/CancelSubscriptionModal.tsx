...
export default function CancelSubscriptionModal() {
    ...
    async function handleCancelSubscription(){
        ...
        if(response?.isOk){
            ...
            updateModal({message: 'Something went wrong, try again later', showModal: true, modalType: 'success'});
        }
                ...
                <View style={styles.scrollContainer}>
                    <View style={styles.mainContainer}>
                        <View style={{flexDirection: 'row', alignItems: 'center', gap: 10}}>
                            ...
                            <CloseButton handlePress={() => setRetainModal({showModal: false, retainModal: null})} />
                        </View>
                        <View style={{padding: 10, backgroundColor: '#FDF7EF', borderRadius: 6, marginVertical: 20,}}>
                            <Text style={{fontSize: 14, color: colors.primary_black}}>
                                Are you sure? After <Text style={{fontWeight: 500}}>{formatIntlDateTime(modalMessage)}</Text>, you will be unable to upload artworks and events or use any of the services provided by Omenai Inc. All artworks uploaded will be suspended until your subscriptions are restarted.
                            </Text>
                        </View>
                        <LongBlackButton
