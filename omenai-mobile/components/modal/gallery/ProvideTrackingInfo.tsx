...
export default function ProvideTrackingInfo() {
    ...
    const checkIsDisabled = () => {
        const areAllFieldsFilled = Object.values({
            tracking_id: trackingInfoForm.tracking_id,
            tracking_link: trackingInfoForm.tracking_link
        }).every((value) => value !== "");
    ...
    const handleSubmit = async () => {
        ...
        let results = await provideTrackingInfo({data: trackingInfoForm, order_id: currentId})
        if(results.isOk){
            setCompleted(true)
            console.log(results)
        }else{
                ...
                {completed ? 
                    ...
                    <View style={{gap: 20}}>
                        <Input
                            ...
                            onInputChange={e => updateTrackingInfoForm('tracking_link', e)}
                            value={trackingInfoForm.tracking_link}
                        />
                        <Input
                            label='Tracking ID'
                            placeHolder='Please provide tracking ID for this package'
                            onInputChange={e => updateTrackingInfoForm('tracking_id', e)}
                            value={trackingInfoForm.tracking_id}
                        />
                    </View>
                }
            </View>
