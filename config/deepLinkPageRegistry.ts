...
const PAGE_REGISTRY: Record<DeepLinkPage, PageDefinition> = {
  ...
  search: {
    kind: "tab",
    roles: ["individual"],
    screenName: screenName.searchResults,
    roleWrapper: "Individual",
  },
  orders: {
    kind: "tab",
...
> = {
  ...
  search: { individual: screenName.searchResults },
  orders: {
...
export const toAppRole = (role: DeepLinkWebRole | DeepLinkAppRole): DeepLinkAppRole =>
  role === "user" ? "individual" : role;

const resolveTabScreenName = (page: DeepLinkTabPage, appRole: DeepLinkAppRole) =>
...
const mapApiParamsToNavParams = (
  ...
  if (page === "payment" && params.id) mapped.id = params.id;
  return mapped;
};
...
const hasRequiredParams = (
  ...
  const required = PAGE_REGISTRY[page].requiredParams;
...
export const resolveDeepLinkTarget = (
  ...
  if (!def || !def.roles.includes(appRole)) {
...
export const sessionAppRole = (options: DeepLinkSessionOptions): DeepLinkAppRole => {
