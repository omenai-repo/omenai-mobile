import type { ArtworkDimensions } from "#types/ar";
...
export function parseArtworkDimensions(dimensions?: {
  height?: string;
  width?: string;
  length?: string;
}): ArtworkDimensions {
  ...
  const height = parseDimensionToMeters(dimensions?.height, DEFAULT_HEIGHT_M);
