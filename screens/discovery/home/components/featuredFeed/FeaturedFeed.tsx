...
export default function FeaturedFeed() {
  ...
  const resolveImage = (item: FeaturedItem) => {
    ...
    if (itemType === "events") {
      return data.cover_image
        ? getPromotionalFileView(data.cover_image, 900)
        : "";
    }
  ...
  const resolveSubtitle = (item: FeaturedItem) => {
    ...
    if (itemType === "events")
      return item.data?.gallery_name || item.data?.gallery?.name || "";
    return "";
  };
              ...
              return (
                ...
                >
                  {imageUri ? (
                    ...
                    <View
                      style={tw`w-full h-[170px] rounded-sm bg-[#0D2040] items-center justify-center`}
                    >
                      <Ionicons
                        name="image-outline"
                        size={24}
                  ...
                  <View style={tw`mt-2`}>
                    <Text
                      style={tw`text-xs font-sans-regular uppercase tracking-widest text-neutral-500`}
                    >
                      {label}
                    </Text>
                    <Text
                    ...
                    {!!subtitle && (
                      <Text
                        numberOfLines={1}
                        style={tw`text-xs text-neutral-600 mt-1`}
                      >
                        {subtitle}
                      </Text>
                    )}
