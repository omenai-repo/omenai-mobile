            ...
            <TouchableOpacity onPress={() => setOpenDropdown(!openDropdown)}>
                <View style={styles.FilterSelectContainer}>
                    <View style={{flex: 1, flexDirection: 'row', alignItems: 'center', gap: 10}}>
                        <Text style={{color: '#616161', fontSize: 16}}>Filter by medium</Text>
                        {filterOptions.medium.length > 0 && (
                            <View style={{paddingHorizontal: 10, paddingVertical: 4, backgroundColor: '#f5f5f5', borderRadius: 20}}><Text style={{fontSize: 12, color: colors.primary_black}}>{filterOptions.medium.length}</Text></View>
                        )}
                    </View>
